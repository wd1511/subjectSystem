...
教师信息
<br>
<%LinkedList<String> teaID = (LinkedList<String>)request.getAttribute("teaID");%>
<%LinkedList<String> teaName = (LinkedList<String>)request.getAttribute("teaName");%>
<%for(int i=0;i<teaID.size();i++){ %>
<%=teaID.get(i) %> <%=" " %><%=" " %><%=" " %><%=teaName.get(i) %>
<input type="button" value="查看个人信息" onclick="javascript:location.href='maTeaInf.action?teacherID=<%=teaID.get(i) %>'"> 
<input type="button" value="删除教师" onclick="javascript:location.href='maDelTea.action?teacherID=<%=teaID.get(i)%>'"> 
<br>
<%} %>
