...
学院：<%=b.get(0) %>-<%=c %>
<br>
专业：<%=a.getFieldID() %>-<%=b.get(1) %>
<br>

<form action = "stuInf1.action?studentID=<%=a.getStudentID() %>" method = "post">
电话号码：<%=a.getPhoneNum() %><%="    " %>新手机号：<%=" " %>:<input type = "text" name = "newPhone"><br>
<input type = "submit" value = "修改信息">
</form>
<br>
<input type="button" value="修改密码" onclick="javascript:location.href='stuInf2.action?studentID=<%=a.getStudentID() %>'"> 
<input type="button" value="返回" onclick="javascript:location.href='chooseSub2.action?studentID=<%=a.getStudentID() %>'"> 
</div>
		</div>
	</div>
