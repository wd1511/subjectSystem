...
<%subject newsubject = (subject)request.getAttribute("newsubject");%>
未将学生<%=newstudent.getStudentID() %>-<%=newstudent.getStudentName() %>踢出课程<%=newsubject.getSubjectID() %>-<%=newsubject.getSubjectName() %>
<br>
<input type="button" value="返回" onclick="javascript:location.href='teaSearchStu.action?studentID=<%=newteacher.getTeacherID()+";"+newsubject.getSubjectID() %>'">
</div>
		</div>
	</div>
